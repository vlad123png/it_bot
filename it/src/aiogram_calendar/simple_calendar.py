...
class SimpleCalendar(GenericCalendar):
    ...
    async def start_calendar(
        ...
        def format_day_string():
            ...
            if self.min_date and date_to_check.date() < self.min_date:
                return superscript(str(day))
            elif self.max_date and date_to_check.date() > self.max_date:
                return superscript(str(day))
            return str(day)
        ...
        # Month nav Buttons
        month_row = [InlineKeyboardButton(
            text="<",
        ...
        # Создание дней по неделям с выделением разрешенных дат для выбора
        ...
        cancel_row = [InlineKeyboardButton(
            ...
            callback_data=SimpleCalendarCallback(
                act=SimpleCalAct.cancel, year=year, month=month, day=1).pack()
        ), InlineKeyboardButton(
            text=self._labels.today_caption,
            callback_data=SimpleCalendarCallback(
                act=SimpleCalAct.today, year=year, month=month, day=datetime.today().day).pack()
        )]
